...
int main()
{
	...
	MyClass b(size2);
	bool equal = a == b; //вызов перегруженного оператора равенства (сравнение множеств)
	...
	b = c;//присваивание объекта через перегруженный оператор присваивания (объект с тем же адресом с новыми данными)
	//b = a = c; //последовательное присваивание через перегруженный оператор (то же адрес, новые данные, с конца выражения)
	b.isInclude(10); //поиск числа в массиве объекта
	b.add(99);//добавление числа в массив 
	a.exclude(2);//удаление числа (всех таких чисел)
	a + b; //оператор сложения массивов
